...
router.get('/gameservers/:id/view', (req, res) =>
{
	if (req.user)
	{
		...
		{
			if (err) { throw err; }
			else { res.render('gameservers/view', { gameserver: row, }); }
		});
	}
...
router.get('/gameservers/:id/config', (req, res) =>
{
	if (req.user)
	{
		...
		{
			if (err) { throw err; }
			else { res.render('gameservers/config', { gameserver: row, }); }
		});
	}
	else { res.render('login'); }
});
				...
				db.get(sql_get_gameserver_by_Id, req.params.id, function (err, row)
				{
					...
					else
					{
						...
						svc.status()
							...
							{
								if (stdout == 'SERVICE_STOPPED') { return; }
								else { return svc.stop(); }
							})
							.then(function (stdout) { return svc.set('AppParameters', appParameters) })
							.catch(function (err) { throw err; });

						res.render('gameservers/view', { gameserver: row });
					}
				});
			}
		});
...
router.post('/gameservers/add', (req, res) =>
{
	if (req.user)
	{
		...
		db.get(sql_get_gameserver_by_InstallationRoute, req.body.installationRoute, function (err, row)
		{
			...
			else
			{
				if (typeof row !== 'undefined') // GameServer already exists
				{
					...
					res.render('gameservers/progress', { gameserver_id: id, });
				}
	...
	if (req.user)
	{
		...
		svc.status()
			.then(function (stdout)
			{
				...
				else { res.sendStatus(200); } // Service is Running or Starting
			})
			.catch(function (err) { throw err; });
	}
	else { res.render('login'); }
	...
	if (req.user)
	{
		...
		svc.status()
			.then(function (stdout)
			{
				...
				else { res.sendStatus(200); } // Service is Stopped or Stopping
			})
			.catch(function (err) { throw err; });
	}
	else { res.render('login'); }
	...
	if (req.user)
	{
		...
		svc.status()
			.then(function (stdout)
			{
				...
				else { res.sendStatus(200); } // Service is Starting, Stopping or Stopped
			})
			.catch(function (err) { throw err; });
	}
	else { res.render('login'); }
...
router.get('/gameservers/:id/getStatus', (req, res) =>
{
	if (req.user)
	{
		var svc = Nssm('SquadServerManager_' + req.params.id, { nssmExe: 'resources/nssm/nssm.exe' });
		svc.status()
			.then(function (stdout) { res.send({ service_status: stdout }); })
			.catch(function () { res.send({ service_status: 'ERROR' }); });
	}
...
router.get('/gameservers/:id/uninstall', (req, res) =>
{
	...
	{
		var svc = Nssm('SquadServerManager_' + req.params.id, { nssmExe: 'resources/nssm/nssm.exe' });
		svc.stop(function () { svc.remove('confirm'); });
